...
export const WorkspacePanel: React.FC = () => {
  ...
  // Plugins installation state (simulating downloadable marketplace)
  ...
  return (
    <div className="w-[380px] bg-[#1a1b1e] border-l border-white/[0.08] flex flex-col h-full text-white animate-in slide-in-from-right duration-200">
        ...
        <div className="flex items-center gap-2">
          {activeView !== 'hub' && (
            <button 
              ...
              className="text-white/60 hover:text-white hover:bg-white/10 p-1 rounded-lg transition-colors cursor-pointer mr-0.5"
            >
      ...
      {activeView === 'hub' && (
        <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-[#1a1b1e]">
          ...
          <div className="space-y-3">
            <h3 className="text-[10px] font-bold text-white/40 uppercase tracking-wider">Installed Plugins</h3>
      ...
      {activeView === 'code' && (
        <>
          <div className="px-4 py-2 bg-[#202124] border-b border-white/[0.06] flex items-center justify-end">
            <button
              onClick={handleCopy}
              className="flex items-center gap-1 text-[10px] font-semibold text-white/60 hover:text-white bg-white/5 hover:bg-white/10 px-2.5 py-1.5 rounded transition-all cursor-pointer border border-white/5"
            >
      ...
      {activeView === 'copilot' && (
        <div className="flex-grow flex flex-col min-h-0 bg-[#1a1b1e]">
          {/* Settings Sub-Bar */}
          <div className="bg-[#202124] border-b border-white/[0.06] text-xs">
          ...
          {/* Quick Shortcuts Bar */}
          <div className="px-4 py-2 border-b border-white/[0.06] flex items-center justify-between gap-1">
            <button
              onClick={() => handleCopilotShortcut('summary')}
              disabled={isCopilotLoading}
              className="flex-1 py-1 bg-white/5 hover:bg-white/10 text-white text-[9px] font-semibold rounded text-center transition-all cursor-pointer"
            >
              Summarize Call
            </button>
            <button
            ...
            {isCopilotLoading && (
              <div className="flex gap-2.5">
                ...
                <div className="bg-white/5 text-white/50 p-3 rounded-lg text-left italic border border-white/[0.04]">
                  Copilot is processing live transcript lines and formulating response...
                </div>
              </div>
            )}
          ...
          <form 
            ...
            className="p-3 bg-[#202124] border-t border-white/[0.08] flex items-center gap-2"
          >
            <input
              type="text"
              value={copilotQuery}
              onChange={(e) => setCopilotQuery(e.target.value)}
              placeholder="Ask Copilot anything about the meeting..."
              disabled={isCopilotLoading}
