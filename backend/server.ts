...
function parseMeetingDetails(meeting: any) {
...
async function sendScheduledMeetingEmails(hostId: string, meeting: any, invitedEmails: string[], rawPasscode: string | null) {
  try {
...
async function sendDirectMessageEmailNotification(senderName: string, recipientEmail: string, messageText: string) {
  try {
    const brevoApiKey = process.env.BREVO_API_KEY;
    if (!brevoApiKey) {
      console.error('Brevo API key is not configured on the server');
...
app.get('/api/chats', requireAuth(), async (req, res) => {
  try {
    ...
    const activeThreads = filteredThreads.map((thread: any) => {
      let recipientEmail = '';
      let recipientName = '';
      let hostEmail = '';
      let hostName = '';
...
app.get('/api/chats/:threadCode', requireAuth(), async (req, res) => {
  try {
...
app.post('/api/chats/message', requireAuth(), async (req, res) => {
  try {
    const userId = getAuth(req).userId;
    ...
    if (!thread) {
      ...
      try {
        const clerkLookup = await clerkClient.users.getUserList({
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('workspace-update', (data: { roomId: string; type: 'markdown' | 'code'; content: string }) => {
